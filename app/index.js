...
function initWalletProvider() {
  ...
  let keystoreDir = path.join('/root/node', 'keys/MBC')
  let keystore
  ...
  let password = fs.readFileSync(path.join('/root/', 'node.pwd')).toString().trim()
...
async function runMain() {
  try {
   ...
   // const isPendingValidator = await consensus.methods.getValidators(web3.utils.toChecksumAddress("0x95ffd5ce53ff771bc9e7404dfb70531e9635ad74")).call()
    //console.log("====PV====")
    //console.log(isPendingValidator);
    //console.log("====PV====")
    if (!isValidator) {
